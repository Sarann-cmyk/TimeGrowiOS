...
Для глибокого розслідування точності (пакетна доставка порогів, дедуплікація, недо- vs
переоблік та серверна трасировка) дивись **`AUTO_TRACKING_RELIABILITY.md`**. Це окремий
робочий документ: він описує поточний компроміс і правила безпечних змін, а не обіцяє, що
поточна дедуплікація вже розв'язала проблему.
...
- `AUTO_TRACKING_RELIABILITY.md` — деталізована карта точності, пакетних порогів і server-side
  трасування; читати перед зміною threshold/dedup/rearm алгоритму.
